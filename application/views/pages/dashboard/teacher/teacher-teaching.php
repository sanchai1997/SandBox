        ...
        <div class="card recent-sales overflow-auto">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col">
                        <h5 style="float: right; padding: 15px;" class="card-title"><a href="forms-teacher-teaching?SchoolID=<?= $_GET['SchoolID'] ?>&&TeacherID=<?= $_GET['TeacherID'] ?>&&EducationYear=<?= $_GET['EducationYear'] ?>&&Semester=<?= $_GET['Semester'] ?>&&PersonnelTypeCode=<?= $_GET['PersonnelTypeCode'] ?>&&PositionCode=<?= $_GET['PositionCode'] ?>" class="btn btn-success"><i class="bi bi-file-earmark-plus"></i> เพิ่มข้อมูล</a></h5>
                    </div>
                </div>
                <table class="table table-borderless datatable">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                <td><?= $TEACHER_TEACHING->TeachingEducationYear; ?></td>
                                <td><?= $TEACHER_TEACHING->TeachingSemester; ?></td>
                                <td><?= $TEACHER_TEACHING->EDUCATION_LEVEL_NAME; ?></td>
                                <td><?= $TEACHER_TEACHING->SUBJECT_TYPE_NAME; ?></td>
                                <td><?= $TEACHER_TEACHING->SUBJECT_GROUP_NAME; ?></td>
                                <td style="text-align: center;">
                                    <a href="edit-forms-teacher-teaching?SchoolID=<?= $_GET['SchoolID'] ?>&&TeacherID=<?= $_GET['TeacherID'] ?>&&EducationYear=<?= $_GET['EducationYear'] ?>&&Semester=<?= $_GET['Semester'] ?>&&PersonnelTypeCode=<?= $_GET['PersonnelTypeCode'] ?>&&PositionCode=<?= $_GET['PositionCode'] ?>&&TeachingEducationYear=<?= $TEACHER_TEACHING->TeachingEducationYear ?>&&TeachingSemester=<?= $TEACHER_TEACHING->TeachingSemester ?>&&TeachingSubjectCode=<?= $TEACHER_TEACHING->TeachingSubjectCode ?>" class="btn btn-warning"><i class="bi bi-pencil-square"></i></a>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a href="<?php echo base_url('delete-teacher-teaching/'  . $_GET['TeacherID'] . '/' .  $_GET['SchoolID'] . '/' .  $_GET['EducationYear'] . '/' .  $_GET['Semester'] . '/' .  $_GET['PersonnelTypeCode'] . '/' .  $_GET['PositionCode'] . '/' . $TEACHER_TEACHING->TeachingEducationYear . '/' . $TEACHER_TEACHING->TeachingSemester . '/' . $TEACHER_TEACHING->TeachingSubjectCode);
