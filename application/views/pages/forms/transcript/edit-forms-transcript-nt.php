<main id="main" class="main">
    <div class="pagetitle">
        <h1 style="padding-bottom: 5px;">เพิ่มข้อมูลผลประเมินคุณภาพผู้เรียน NT: ใบแสดงผลการศึกษาชุดที่
            ...
            <div class="col-lg-9">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="row g-3" action="<?php echo base_url('update-transcript-nt/' . $_GET['SchoolID'] . '/' . $_GET['StudentReferenceID'] . '/' . $_GET['EducationYear'] . '/' . $_GET['Semester'] . '/' . $_GET['GradeLevelCode'] . '/' . $_GET['StudentID'] . '/' . $_GET['TranscriptSeriesNumber'] . '/' . $_GET['TranscriptNumber'] . '/' . $_GET['NTEducationYear'] . '/' . $_GET['NTSemester'] . '/' . $_GET['NTGradeLevelCode']); ?>" method="POST" id="Transcript" enctype="multipart/form-data">
                            ...
                            $result = $this->db->query('SELECT *  FROM TRANSCRIPT_NT 
                                ...
                                AND NTEducationYear = ' . $_GET['NTEducationYear'] . '
                                AND NTSemester = ' . $_GET['NTSemester'] . '
                                AND NTGradeLevelCode = ' . $_GET['NTGradeLevelCode'] . '
                                ');
                            foreach ($result->result() as $TRANSCRIPT_NT) {
                            ?>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="number" class="form-control" maxlength="4" name="NTEducationYear" id="NTEducationYear" value="<?= $TRANSCRIPT_NT->NTEducationYear ?>" disabled>
                                        <label for="NTEducationYear">ปีการศึกษา<font color="red"> *</font></label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="number" class="form-control" maxlength="1" name="NTSemester" id="NTSemester" value="<?= $TRANSCRIPT_NT->NTSemester ?>" disabled>
                                        <label for="NTSemester">ภาคเรียน<font color="red"> *</font></label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="NTGradeLevelCode" id="NTGradeLevelCode" aria-label="NTGradeLevelCode" disabled>
                                            <?php
                                            $result = $this->db->query('SELECT * FROM CLS_GRADE_LEVEL');
                                            foreach ($result->result() as $GRADE_LEVEL) {
                                            ?>
                                                <option <?php if ($TRANSCRIPT_NT->NTGradeLevelCode == $GRADE_LEVEL->GRADE_LEVEL_CODE) {
                                                            echo 'selected';
                                ...
                                <div class="col-md-3">
                                    <div class="form-floating">
                                        <input type="number" class="form-control" step="0.01" name="NTMathScore" id="NTMathScore" value="<?= $TRANSCRIPT_NT->NTMathScore ?>">
                                        <label for="NTMathScore">คะแนนสอบด้านคณิตศาสตร์<font color="red"> *</font></label>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-floating">
                                        <input type="number" class="form-control" step="0.01" name="NTThaiLanguageScore" id="NTThaiLanguageScore" value="<?= $TRANSCRIPT_NT->NTThaiLanguageScore ?>">
