...
class Fm_evaluation extends _sandboxcontroller
{
    ...
    public function print_excel()
    {
        ...
        $sheet->setCellValue('A1', 'รหัสตัวชี้วัด');
        $sheet->setCellValue('B1', 'ชื่อตัวชี้วัด');
        $sheet->setCellValue('C1', 'คำอธิบายเกณฑ์');
        $sheet->setCellValue('D1', 'คะแนนการผ่านเกณฑ์(%)');

        $ASSESSMENT_CRITERIA = $this->db->query("SELECT * FROM ASSESSMENT_CRITERIA WHERE DeleteStatus = 0")->result_array();
        $x = 2;
        foreach ($ASSESSMENT_CRITERIA as $row) {
           
            
            $sheet->setcellValue('A' . $x, $row['CriteriaID']);
            $sheet->setCellValue('B' . $x, $row['CriteriaName']);
            $sheet->setCellvalue('C' . $x, $row['CriteriaDescription']);
            $sheet->setCellvalue('D' . $x, $row['CriteriaPassingScorePercentage']);
            $x++;
        }
